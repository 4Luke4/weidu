...
or & \DEFINE{OUTER_WHILE} \ttref{value} \t{BEGIN}
  ...
  \\
or & \ttref{GOTO} label &
  Continue execution at the visible action label written as
  \t{::label::}. Label names are non-empty, case-sensitive literals and are
  not evaluated for variables. See ``Control-flow scope'' below. \\
or & \t{::}label\t{::} &
  Declare an action label for \ttref{GOTO}. The label itself has no run-time
  effect. \\
or & \DEFINE{BREAK} &
...
or & \DEFINE{WHILE} \ttref{value} \t{BEGIN}
  ...
  \\
or & \ttref{GOTO} label &
  Continue execution at the visible patch label written as
  \t{::label::}. Label names are non-empty, case-sensitive literals and are
  not evaluated for variables. See ``Control-flow scope'' below. \\
or & \t{::}label\t{::} &
  Declare a patch label for \ttref{GOTO}. The label itself has no run-time
  effect. \\
or & \ttref{BREAK} &
...
\label{sec-control-flow}\subsection{Control-flow scope}

The \ttref{GOTO} and \ttref{BREAK} statements are available in both action
and patch code. An action \ttref{GOTO} can target only an action label, and a
patch \ttref{GOTO} can target only a patch label. A label is visible throughout
the action or patch list that declares it and throughout nested control-flow
lists of the same kind. Thus, a jump may leave a nested conditional or loop,
but it may not jump into a nested list.

Function and macro bodies, included files, embedded lists of the other kind,
and patch lists that operate on a separate buffer form new control-flow
regions. Labels and jumps cannot cross those boundaries. A label name cannot
duplicate another visible label name. WeiDU rejects duplicate labels, jumps
without a visible target, empty label names, and \ttref{BREAK} statements
outside a suitable loop while parsing the file, before executing its code.
\ttref{ACTION_TRY} and \ttref{PATCH_TRY} do not catch these control transfers.

For example, a backward jump can express a small action-state machine, while
\ttref{BREAK} avoids an extra sentinel variable when a loop has found its
result:
